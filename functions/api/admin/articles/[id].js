...
import { canDraft, canApproveSend, audit, json, forbidden, bad } from "../_lib.js";
...
export async function onRequestPatch(context) {
  ...
  const a = await load(env, params.id);
  if (!a) return json({ ok: false, error: "Not found" }, 404);
  if (a.status === "sent") return bad("This issue has already been sent and can't be changed.");

  let body;
  try { body = await request.json(); } catch { return bad("Invalid JSON."); }

  // 1) content edits (drafters+)
  const fields = [];
  const binds = [];
