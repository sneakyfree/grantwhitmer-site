...
export async function onRequestPost(context) {
  ...
  const outline = (body.podcast_outline || "").trim().slice(0, 12000) || null;
  if (!subject) return bad("A subject line is required.");
  if (!html) return bad("The issue body can't be empty.");

  const res = await env.DB.prepare(
    `INSERT INTO articles (subject, preview, body_html, status, created_by, type, podcast_outline)
     VALUES (?1, ?2, ?3, 'draft', ?4, ?5, ?6)`
  ).bind(subject, preview, html, data.user.email, type, outline).run();
